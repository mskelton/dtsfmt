...
pub fn get_specs_in_dir(path: &Path) -> Vec<(PathBuf, Spec)> {
    ...
    for (file_path, text) in spec_files {
        ...
        let lower_case_file_path =
            file_path.to_string_lossy().to_ascii_lowercase();

        let path_has_only = lower_case_file_path.contains("_only.txt")
            || lower_case_file_path.contains("_only/")
            || lower_case_file_path.contains("_only\\");

        let is_only_file =
            path_has_only && !specs.iter().any(|spec| spec.is_only);
